...
/**
 * 启动 CS:GO 播放 Demo。优先库内 id，否则按 path。
 * @param {{ id?: number | string | null, path?: string | null }} opts
 */
export async function playDemoInCsgo({ id = null, path = null, playerAliases = null } = {}) {
  const body = {
    ...(playerAliases && Object.keys(playerAliases).length
