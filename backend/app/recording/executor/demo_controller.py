...
# Non-Windows: stubs
try:
    from ...win_csgo_console import inject_console_sequence, send_csgo_vk_tap
except ImportError:
    def inject_console_sequence(lines): pass
    def send_csgo_vk_tap(vk: int) -> bool: return False
...
async def gototick(tick: int, verify_tolerance_ticks: int = 32) -> None:
    """
    Send demo_gototick command and wait for CSGO to seek.

    Since we cannot read the current demo tick from outside CSGO,
    ...
    # Wait for seek to complete (CSGO async disk read)
    await asyncio.sleep(1.2)

async def demo_resume() -> None:
    """Send demo_resume to CSGO."""
    try:
...
async def demo_pause_silent() -> None:
    ...
    """
    try:
        ok = await asyncio.to_thread(send_csgo_vk_tap, _VK_NUMPAD5)
        if not ok:
...
async def demo_resume_silent() -> None:
    ...
    """
    try:
        ok = await asyncio.to_thread(send_csgo_vk_tap, _VK_NUMPAD6)
        if not ok:
